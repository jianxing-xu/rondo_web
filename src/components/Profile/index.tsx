...
export const ProfilePanel: React.FC<IProfilePanel> = ({ children }) => {
  ...
  const { loading: listLoading, data: songs, fetching: songsFetching } = useFetch(userSongs, { user_id: u }, { init: false })
  useEffect(() => {
    if (!!u) {
      fetching();
      songsFetching();
  ...
  const goRoom = () => {
    ...
    hdieAll();
    changeRoom(data.room?.room_id);
  }
  return <RightPanelWrapper>
    <div className="absolute top-0 bottom-0 left-0 right-0 flex flex-col h-full">
      <Spin spinning={loading}>
        <div style={{ height: 220 }} className="flex flex-col items-center justify-between w-full p-4 pb-1 space-y-2 text-center bg-bgc">
        ...
        <div className="absolute top-0 bottom-0 left-0 right-0 overflow-y-auto m_scroll">
          {
            songs?.list?.map((item: any) => (
              <div key={item?.song_id} className="flex items-center p-2 bg-bg-light">
                <div className="flex flex-col items-start justify-between space-y-2">
                  <span>{item?.song_name ?? "--"}</span>
                  <span>{item?.song_singer ?? "--"}</span>
                </div>
                <div className="ml-auto text-primary">{item?.song_play ?? "--"}次</div>
              </div>
            ))
          }
        </div>
      ...
      <div className="flex justify-end w-full bg-bgc" style={{ height: "8%" }}>
        <div className="self-center px-2 mr-2 border cursor-pointer" onClick={goRoom}>去TA房间看看</div>
      </div>
    </div>
  </RightPanelWrapper>
}